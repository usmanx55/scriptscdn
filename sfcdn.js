...
document.addEventListener("DOMContentLoaded",()=>{const body=document.body,menuButton=document.querySelector(".menu_button"),primaryNav=document.querySelector(".primary_navigation");if(!menuButton||!primaryNav)return;let isMobile=matchMedia("(max-width: 896px)").matches;const submenuCache=new Map();const init=()=>{menuButton.ariaExpanded="false";primaryNav.querySelectorAll("li.has_sub > a").forEach(link=>{const sub=link.parentElement.querySelector(".submenu_drawer");if(sub){submenuCache.set(link,{element:sub,isOpen:!1});link.ariaExpanded="false";if(isMobile)sub.style.display="none"}})};const handleAction=(e)=>{const link=e.target.closest("li.has_sub > a");if(!link||!submenuCache.has(link))return;e.preventDefault();const{element:sub,isOpen}=submenuCache.get(link);requestAnimationFrame(()=>{const newState=!isOpen;submenuCache.forEach((data,key)=>{if(key===link){data.isOpen=newState;link.ariaExpanded=newState;key.parentElement.classList.toggle("open",newState);if(isMobile)sub.style.display=newState?"block":"none"}else if(data.isOpen){data.isOpen=!1;key.ariaExpanded="false";key.parentElement.classList.remove("open");if(isMobile)data.element.style.display="none"}})})};const toggleMenu=(e)=>{e.preventDefault();body.classList.toggle("mobile-menu-open");menuButton.ariaExpanded=body.classList.contains("mobile-menu-open")};const resizeHandler=()=>{const newMobile=matchMedia("(max-width: 896px)").matches;if(isMobile!==newMobile){isMobile=newMobile;submenuCache.forEach(({element})=>{element.style.display=isMobile?"none":""})}};init();primaryNav.addEventListener("click",handleAction);menuButton.addEventListener("click",toggleMenu);if(window.ResizeObserver){new ResizeObserver(resizeHandler).observe(document.documentElement)}else{addEventListener("resize",resizeHandler,{passive:!0})}})

// themeswitch
(function(){const K='blogTheme',T={D:'default',DA:'dark',L:'light'};let r=null,m=null,c=null;function g(){try{return localStorage.getItem(K)||T.D}catch(e){return T.D}}
function a(t,i=!1){if(t===c&&!i)return;c=t;requestAnimationFrame(()=>{r.setAttribute('data-theme',t);if(t!==T.D||!i)try{localStorage.setItem(K,t)}catch(e){}})}
function i(){if(r)return;r=document.documentElement.querySelector('.nB')||document.body;m=window.matchMedia('(prefers-color-scheme: dark)');document.addEventListener('click',e=>{const t=e.target.closest('[onclick*="setTheme"],[data-theme-value]');if(!t)return;e.preventDefault();let h=t.getAttribute('data-theme-value');if(!h){const m=t.getAttribute('onclick')?.match(/setTheme\(['"](.+?)['"]\)/);if(m&&m[1])h=m[1]}
if(h)a(h);});try{m.addEventListener('change',()=>{if(c===T.D)requestAnimationFrame(()=>r.setAttribute('data-theme',T.D));},{passive:!0})}catch(e){m.addListener(()=>{if(c===T.D)requestAnimationFrame(()=>r.setAttribute('data-theme',T.D));})}
a(g(),!0)}
try{const s=g();const root=document.documentElement.querySelector('.nB')||document.body;if(root)root.setAttribute('data-theme',s);}catch(e){}
if('requestIdleCallback' in window)requestIdleCallback(i,{timeout:1000});else setTimeout(i,20);window.setTheme=t=>{if(t&&(t===T.D||t===T.DA||t===T.L))a(t);return!1}})();
