...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const body = document.body, 
        ...
        primaryNav = document.querySelector(".primary_navigation");
  
  if (!menuButton || !primaryNav) return;

  // Pre-optimized state management
  let isMobile = matchMedia("(max-width: 896px)").matches;
  const submenuCache = new Map();

  // One-time DOM initialization
  const init = () => {
    menuButton.ariaExpanded = "false";
    primaryNav.querySelectorAll("li.has_sub > a").forEach(link => {
      const sub = link.parentElement.querySelector(".submenu_drawer");
      if (sub) {
        submenuCache.set(link, {
          element: sub,
          isOpen: false
        });
        link.ariaExpanded = "false";
        if (isMobile) sub.style.display = "none";
      }
    });
  };
  
  // Zero-DOM-query interaction handler
  const handleAction = (e) => {
    const link = e.target.closest("li.has_sub > a");
    if (!link || !submenuCache.has(link)) return;
    
    e.preventDefault();
    const { element: sub, isOpen } = submenuCache.get(link);
    
    requestAnimationFrame(() => {
      const newState = !isOpen;
      submenuCache.forEach((data, key) => {
        if (key === link) {
          data.isOpen = newState;
          link.ariaExpanded = newState;
          key.parentElement.classList.toggle("open", newState);
          if (isMobile) sub.style.display = newState ? "block" : "none";
        } else if (data.isOpen) {
          data.isOpen = false;
          key.ariaExpanded = "false";
          key.parentElement.classList.remove("open");
          if (isMobile) data.element.style.display = "none";
        }
      });
    });
  };
  ...
  const toggleMenu = (e) => {
    ...
    menuButton.ariaExpanded = body.classList.contains("mobile-menu-open");
  };

  // Modern ResizeObserver + fallback
  const resizeHandler = () => {
    const newMobile = matchMedia("(max-width: 896px)").matches;
    if (isMobile !== newMobile) {
      isMobile = newMobile;
      submenuCache.forEach(({ element }) => {
  ...
  init();
  primaryNav.addEventListener("click", handleAction);
  menuButton.addEventListener("click", toggleMenu);
  
  if (window.ResizeObserver) {
    new ResizeObserver(resizeHandler).observe(document.documentElement);
  } else {
